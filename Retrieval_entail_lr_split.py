...
def entail_check(images, texts, data_loader):
    assert len(images) == len(texts)
    trainset = data_loader.dataset
    img2txt_entail = trainset.img2txt_entail
    goldens = trainset.goldens
    crush = 0
    for i in range(len(images)):
        image_name = images[i]
        if image_name not in img2txt_entail.keys():
            continue
        entail_pool = img2txt_entail.get(image_name, []) + goldens.get(image_name, [])
        for j in range(len(texts)):
            if i!=j:
                if texts[j] in entail_pool:
                    entail_candidates = img2txt_entail.get(images[j], [])
                    if len(entail_candidates):
                        texts[j] = random.sample(entail_candidates, 1)[-1]
                    crush += 1
    return texts, crush
...
def train(model, data_loader, optimizer, epoch, device, scheduler, config):
    ...
    metric_logger.add_meter('lr_low', utils.SmoothedValue(
        window_size=1, fmt='{value:.8f}'))
    ...
    optimizer.zero_grad()
    for i, (image, text, extra_entail, image_names) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
